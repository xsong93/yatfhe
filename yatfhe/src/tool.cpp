    ...
    for (auto l = 0; l < in.l; l++) {
        ...
        for (int i = 0; i < in.rlwes[l].k; i++) {
            ...
            for (int j = 0; j < in.rlwes[l].b.N; j++) {
                printf("%d,%d:%s%lu%s  ", i, j, ANSI_COLOR_YELLOW, in.rlwes[l].a[i].coeffs[j], ANSI_COLOR_RESET);
            }
            cout << "] ";
        }
        cout << endl << "b: [";
        for (int j = 0; j < in.rlwes[l].b.N; j++) {
            printf("%d:%s%lu%s  ", j, ANSI_COLOR_YELLOW, in.rlwes[l].b.coeffs[j], ANSI_COLOR_RESET);
