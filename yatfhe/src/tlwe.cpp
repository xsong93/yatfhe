...
void symEncTlwe(Tlwe& tlweSample, const Torus message, const TlweKey& key) {
    int64_t tmp = 0;
    for (auto i = 0; i < key.n; i++) {
        tlweSample.a[i] = static_cast<LweTorus>(uniformTorusDistrib(LWE_MIN, LWE_MAX)(rng));
        if (key.s[i] != 0) {
            tmp += static_cast<int64_t>(tlweSample.a[i]) * key.s[i];
        }
    }
    Torus muE = addTUniformNoise(message, key.errorB, LWE_Q);
    tlweSample.b = static_cast<LweTorus>(addTorus(LWE_Q, muE, static_cast<Torus>(longModP(tmp, LWE_Q))));
}
...
void rescaleTlweFromTorus(Tlwe& output, const Tlwe& input) {
    output.b = static_cast<LweTorus>(modSwitchFromTorusGeneral(input.b, LWE_Q, TORUS_Q));
    for (auto i = 0; i < input.n; i++) {
        output.a[i] = static_cast<LweTorus>(modSwitchFromTorusGeneral(input.a[i], LWE_Q, TORUS_Q));
    }
}
...
void addTlwe(Tlwe& output, const Tlwe& input1, const Tlwe& input2) {
    for (auto i = 0; i < output.n; i++) {
        output.a[i] = static_cast<LweTorus>(addTorus(LWE_Q, input1.a[i], input2.a[i]));
    }
    output.b = static_cast<LweTorus>(addTorus(LWE_Q, input1.b, input2.b));
}

void subTlwe(Tlwe& output, const Tlwe& input1, const Tlwe& input2) {
    for (auto i = 0; i < output.n; i++) {
        output.a[i] = static_cast<LweTorus>(subTorus(LWE_Q, input1.a[i], input2.a[i]));
    }
    output.b = static_cast<LweTorus>(subTorus(LWE_Q, input1.b, input2.b));
}

// output -= input
void subTlweInPlace(Tlwe& output, const Tlwe& input) {
    for (auto i = 0; i < output.n; i++) {
        output.a[i] = static_cast<LweTorus>(subTorus(LWE_Q, output.a[i], input.a[i]));
    }
    output.b = static_cast<LweTorus>(subTorus(LWE_Q, output.b, input.b));
}

void multTlwe(Tlwe& output, const int scalar) {
    for (auto i = 0; i < output.n; i++) {
        output.a[i] = static_cast<LweTorus>(multTorus(LWE_Q, output.a[i], scalar));
    }
    output.b = static_cast<LweTorus>(multTorus(LWE_Q, output.b, scalar));
}
...
void inverseTlwe(Tlwe& tlwe) {
    tlwe.b = static_cast<LweTorus>(modMulQ(static_cast<Torus>(tlwe.b), static_cast<Torus>(-1), LWE_Q));
    for (auto i = 0; i < tlwe.n; i++) {
        tlwe.a[i] = static_cast<LweTorus>(modMulQ(static_cast<Torus>(tlwe.a[i]), static_cast<Torus>(-1), LWE_Q));
    }
}
