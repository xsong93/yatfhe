...
namespace NttNative14 {
    ...
    Ntt14 modADDscale(Ntt14 a, Ntt14 b) {
        Ntt14 temp = ((MOD - a) > b) ? (a + b) : (a + b - MOD);
    ...
    Ntt14 modMULT(Ntt14 a, Ntt14 b) {
        uint32_t a1 = a;
        uint32_t b1 = b;
        return static_cast<Ntt14>(a1 * b1 % MOD);
    }

    void genTW_ROM(TwRom &tw_rom) {
        auto w_n = (tw_rom.N) >> 1;
        auto phi_n = tw_rom.N;
        Ntt14 w_q = Ntt14((MOD - 1) / (w_n << 1));
        Ntt14 phi_q = Ntt14((MOD - 1) / (phi_n << 1));
        Ntt14 temp = 0;
        for (int i = 0; i < w_n; i++) {
            temp = POW(PRIM_ROOT, Ntt14(i * w_q));
            tw_rom.w_rom[i] = temp;
            tw_rom.inv_w_rom[i] = modINV(temp);
        }
        for (int j = 0; j < phi_n; j++) {
            temp = POW(PRIM_ROOT, Ntt14(j * phi_q));
    ...
    void applyNtt(Ntt14Polynomial &RES, const IntPolynomial &IN) {
        ...
        for (int i = 0; i < N; i++) {
            if (IN.coeffs[i] >= 0) {
                ...
                format_input.coeffs[i] = Ntt14(IN.coeffs[i] + MOD);
            }
        }
    ...
    void applyNttPoly8(Ntt14Polynomial &RES, const Int8Polynomial &IN) {
        ...
        for (int i = 0; i < N; i++) {
            if (IN.coeffs[i] >= 0) {
                ...
                format_input.coeffs[i] = static_cast<Ntt14>(IN.coeffs[i] + MOD);
            }
        }
        DIT_NR14(RES, format_input);
    }
    ...
    void applyInttPoly8(Int8Polynomial &RES, const Ntt14Polynomial &IN) {
        ...
        for (int i = 0; i < N; i++) {
            if (res.coeffs[i] >= HALF_MOD) {
                temp_ntt = static_cast<int16_t>(res.coeffs[i] - MOD);
            } else {
                temp_ntt = static_cast<int16_t>(res.coeffs[i]);
            }
            temp_poly = static_cast<uint16_t>(temp_ntt & NTT_MASK);
            if (temp_poly >= POLY_MAX8) {
    ...
    void initGlobalParamsNtt(int N) {
        ...
        TwParam::initTwParam(NWC_TW14, depth);
        TwParam::initTwParam(NWC_ITW14, depth);
        TwRom::initTwRom(TW_ROM14, N);
        genTW_ROM(TW_ROM14);
        genNWCparam(NWC_TW14, N, TW_ROM14, STR_NTT);
        genNWCparam(NWC_ITW14, N, TW_ROM14, STR_INTT);
    }

    void modularAccumulate14(std::vector<Ntt14> &coeffsB, const std::vector<Ntt14> &coeffsA,
                             const std::vector<Ntt14> &coeffsS) {
