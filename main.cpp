...
int main(int argc, char **argv) {
    ...
    TlweKey* keyTlwe = tlweNewBinaryKey(YatfheParameters::n, YatfheParameters::lwe_std_dev);
    TlweKey* keyTlweOut = tlweNewBinaryKey(YatfheParameters::N, YatfheParameters::lwe_std_dev);
    TrlweKey* keyTrlwe = trlweNewBinaryKey(YatfheParameters::N, YatfheParameters::k);
//    trlwe_extract_tlwe_key(key_tlwe_out, key_trlwe);
    TrgswKey* trgswKey = trgswInitKey(keyTrlwe, YatfheParameters::l, YatfheParameters::bg_bit);
...
//    TRLWE lut_c = trlwe_new_noiseless_trivial_sample(poly_res, k, N);
    BootstrappingKey* bsKey = newBootstrappingKey(trgswKey, keyTlwe, 1);
...
//    );
//#endif
    deleteLweKey(keyTlwe);
    deleteLweKey(keyTlweOut);
    deleteRlweKey(keyTrlwe);
    delete timer;
    return 0;
}
