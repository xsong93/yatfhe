...
TEST(NttIntt32Test, NttIntt32Test) {
    string a = "[0:6, 1:0, 2:10, 3:7, 4:2]";
    std::vector<int> aPrime = extractValues(a);
    printArray(aPrime, "a");

    const int N = aPrime.size();
    IntPolynomial a1{N};
    IntPolynomial ntt{N};
    IntPolynomial intt{N};
    for (auto i = 0; i < N; i++) {
        a1.coeffs[i] = aPrime[i];
    }
    applyNtt32(ntt, a1, 9, 11);
...
TEST(NttInttTest, NttInttTest) {
    cout << modPow(2, 811, 1000000007) << endl;
    cout << reciprocal(811, 1000000007) << endl;
