...
TEST(HEXL, NTT_ROT) {
    ...
    printBanner("HEXL.NTT_ROT");
}

TEST(HEXL, NTT_RECOMP) {
    YatfheParameters param {};
    param.N = 4096;
    param.l = 4;
    param.radixBits = 8;
    ...
    for (auto j = 0; j < param.N; j++) {
        ori.coeffs[j] = genIntUniformDist(TORUS_MIN, TORUS_MAX);
        signedGadgetDecomposition(decomp[j], ori.coeffs[j], param);
    }
    ...
    printArray(recomp.coeffs, "recomp");
    ASSERT_EQ(recomp.coeffs, ori.coeffs);
    printBanner("HEXL.NTT_RECOMP");
}
