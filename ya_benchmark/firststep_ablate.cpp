...
int main(int argc, char** argv) {
    ...
    for (int rep = 0; rep < reps; rep++) {
        ...
        for (int step = 0; step < 2; step++) {
            ...
            (runRestruct ? restructSteady : plainSteady)
                .push_back(duration_cast<microseconds>(t1 - t0).count() / 1000.0);
        }
    }
    ...
    const std::string path = yabench::benchOutPath("firststep_results.json");
